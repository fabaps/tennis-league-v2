...
function killProcessOnPort(port: number): void {
    ...
    try {
        ...
        try {
            ...
            console.log(`✅ Puerto ${port} está libre (comando falló, probablemente no encontró procesos).`);
            // console.debug(`   (Error al buscar PID: ${error.message})`); // Descomentar para depuración
            return;
        }
        ...
        pids.forEach(pid => {
            try {
                ...
                if (killError.message.includes('process does not exist') || killError.message.includes('not found') || killError.message.includes('No such process')) {
                     console.log(`ℹ️ PID ${pid} ya no existía.`);
                } else {
                     console.warn(`⚠️ No se pudo detener PID ${pid}: ${killError.message}`);
                }
            }
        });
        if (killedCount > 0) {
        ...
        console.error(`❌ Error crítico ejecutando comandos para puerto ${port}: ${outerError.message}`);
...
function startFirebaseEmulators(): void {
    ...
    try {
        ...
        console.log("\nℹ️ Emuladores de Firebase detenidos.");
